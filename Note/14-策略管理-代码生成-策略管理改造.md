...
![策略管理表结构](NodeAssets/策略管理表结构.png)
...
将“策略方案”是一个折扣数字。所以要将它改为数字文本框，

并要求只能输入**大于 1，小于 100 的数字。不能是小数**。
...
const handlePolicyInfo = row => {
  ...
  listVm(loadAllParams).then(res => {
    ...
    policyinfoOpen.value = true // 关闭对话框
  })
}
</script>
```
...
在设备管理页面中，点击“操作”列的“策略”按钮，对设备设置一个固定策略，用于营销。
...
在修改设备记录中的策略 id 时，会引起空指针异常，原因是没有传入点位 id。
