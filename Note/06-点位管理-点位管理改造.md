...
## 三、resultMap、association、collection 标签
...
#### 3.3.1. collection 标签的使用

在 MyBatis 的 `<resultMap>` 里，`<collection>` 子标签用于处理**一对多**或**多对多**关系的数据映射。
...
MyBatis 映射（resultMap + collection）
...
- `<collection>` 作用于 `orders` 属性，表示 `User` 对象的 **订单集合**。
- `ofType="com.example.Order"` 指定 `orders` 集合里的元素类型是 `Order`。
- `order_id` 和 `order_number` 对应 `Order` 实体的 `id` 和 `orderNumber`。
- SQL 语句使用 `LEFT JOIN`，确保即使用户没有订单也能正确查询出数据。

这样，查询 `getUserWithOrders(1)` 时，MyBatis 会自动把 `Order` 数据映射到 `User.orders` 列表中。

这个方法适用于一对多、多对多的映射，能减少手动处理嵌套结果的代码量。

### 3.4.MyBatis 映射文件标签使用总结
