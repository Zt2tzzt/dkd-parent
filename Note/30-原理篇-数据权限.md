...
我们不希望一个部门的员工能够访问另一个部门的敏感信息，

数据权限的场景：

1. **部门级权限**：市场部的员工应该只能访问到销售相关的数据，确保他们只能触及自己部门的信息。
...
用户则通过他们的角色来继承这些权限，进而决定他们能访问哪些系统资源。
...
> 提示：默认系统管理员 `admin`，拥有所有数据权限 `（userId=1）`；默认角色，拥有所有数据权限（如不需要数据权限不用设置数据权限操作）

## 二、数据权限表结构

若依系统的数据权限设计，主要通过用户、角色、部门表建立关系，实现对数据的访问控制：
...
其作用相当于在一个 select 语句后面，拼接一个 and 条件语句，来实现查询限制，如下所示：
...
结果很明显，多了如下语句。通过角色部门表`（sys_role_dept）`完成了自定义类型的数据权限过滤
...
2.在 MyBatis 的 XML 映射文件里的 SQL 语句末尾加 `${params.dataScope}` 实现数据范围过滤。

3.要求：

1. 实现数据权限的实体类，需要集成 `BaseEntity` 类。
2. 实现数据权限的业务表，需要有 `dept_id` 和 `user_id` 这两个字段。
...
但是，并不是所有人都应该看到所有的日志数据。所以，我们需要根据用户的角色，来控制他们能查看的数据范围。

![登录日志](NodeAssets/登录日志.png)

### 4.1.添加权限注解

在 Service 层的 SysLogininforServiceImpl 实现类里的 `selectLogininforList` 方法傻姑娘，加上 `@DataScope(deptAlias = "d", userAlias = "u")` 注解。
