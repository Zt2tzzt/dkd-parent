...
![货道关联商品效果图](NodeAssets/货道关联商品效果图.png)
...
可见 UPDATE 语句，写在了 `<foreach>` 标签里，意味着一次请求，可能执行多条 UPDATE 语句。

> MySQL 连接，一次请求，默认只支持单条 sql 的执行，如果要运行多条 sql 的执行，要加入一个参数 `allowMultiQueries=true`
>
> ```yaml
> # 数据源配置
> spring:
